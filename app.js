...
function renderState(m){
  ...
  if (chip){
    const cls = (window.STATE_CLASS&&STATE_CLASS[st]) || 'neutral';
    chip.className = 'chip state '+cls; chip.textContent=st.replace('_',' ');
    const hs=$('#houseStatus'); if(hs) hs.textContent = m.notte ? 'Notte' : 'Giorno';
  }
  const sb=$('#stateBanner'); if(sb){ const map={'COMFY_DAY':'sb-comfy-day','COMFY_NIGHT':'sb-comfy-night','SECURITY_DAY':'sb-security-day','SECURITY_NIGHT':'sb-security-night'}; sb.className='state-banner '+(map[st]||'sb-security-day'); sb.textContent=st.replace('_',' '); }
}

function renderPeople(m){
  const arr=Array.isArray(m.people)?m.people:[]; const n=arr.filter(p=>p.onlineSmart||p.onlineRaw).length;
  const ppl=$('#peopleOnline'); if(ppl) ppl.textContent=n+' in casa';
  const chips=$('#peopleChips'); if(chips){ chips.innerHTML=''; arr.forEach(p=>{ const on=(p.onlineSmart||p.onlineRaw); const el=document.createElement('div'); el.className='chip'; el.innerHTML=`<span class="dot ${on?'on':'off'}"></span>${p.name}`; chips.appendChild(el); }); }
}

function setWeatherUI(obj){
  const temp=$('#tempValue'); if(temp && typeof obj.tempC==='number') temp.textContent=Math.round(obj.tempC);
  const hum=$('#humValue'); if(hum && typeof obj.humidity==='number') hum.textContent=Math.round(obj.humidity);
  const wt=$('#weatherTemp'); if(wt && typeof obj.tempC==='number') wt.textContent=Math.round(obj.tempC)+'°C';
  const wk=$('#windKmh'); if(wk && typeof obj.windKmh==='number') wk.textContent=Math.round(obj.windKmh)+' km/h';
  const emoji=$('#weatherEmoji'); if(emoji && obj.icon!=null){ const m=(window.mapWeatherCode?mapWeatherCode(obj.icon):{icon:''}); emoji.textContent=(m&&m.icon)||''; }
...
async function fetchWeatherClient(){
  ...
  const url='https://api.open-meteo.com/v1/forecast?latitude='+encodeURIComponent(lat)+'&longitude='+encodeURIComponent(lon)+'&current=temperature_2m,relative_humidity_2m,weather_code,wind_speed_10m&timezone='+encodeURIComponent(tz||'Europe/Rome');
  const r=await fetch(url); if(!r.ok) throw new Error('Open-Meteo HTTP '+r.status); const js=await r.json();
  const cur=(js&&js.current)||{};
  return { tempC: (typeof cur.temperature_2m==='number')?cur.temperature_2m:null,
           humidity: (typeof cur.relative_humidity_2m==='number')?cur.relative_humidity_2m:null,
           icon: (cur.weather_code!=null)?String(cur.weather_code):'',
           windKmh: (typeof cur.wind_speed_10m==='number')?cur.wind_speed_10m:null,
           provider:'Open‑Meteo' };
}

function renderWeather(m){
  const w=(m&&m.weather)||{}; setWeatherUI({ tempC:w.tempC, humidity:w.humidity, icon:w.icon, provider:w.provider, windKmh:w.windKmh});
}

async function ensureWeatherFallback(model){
  try{
    const force=!!(CONFIG&&CONFIG.WEATHER&&CONFIG.WEATHER.forceClient);
    const has=(model&&model.weather&&(typeof model.weather.tempC==='number' || typeof model.weather.humidity==='number'));
    if(has && !force){ setWeatherUI(model.weather); return; }
    const w=await fetchWeatherClient(); setWeatherUI(w); console.log('[UI] weather (client) =', w);
...
function favIconPath(id){
  const map={
    'tapparelle':'icons/st/tapparelle.svg',
    'piante':'icons/st/fav-good-morning.svg',
    'vacanza':'icons/st/fav-good-night.svg',
    'override':'icons/st/fav-power.svg'
  }; return map[id]||'icons/st/fav-play.svg';
}
...
function renderFavorites(m){
  ...
  (CONFIG.FAVORITES||[]).forEach(f=>{
    ...
    const tile=document.createElement('div'); tile.className='fav card-glow'+(isOn?' active':'');
    tile.innerHTML = `
      <div class="top-row">
        <div class="label-col">
        ...
        <button class="action-btn" data-id="${f.id}" aria-label="${f.kind==='action'?'Esegui':'Accendi/Spegni'}">
          <img src="${f.kind==='action'?'icons/st/fav-play.svg':'icons/st/fav-power.svg'}" alt="btn"/>
        </button>
      </div>`;
    ...
    tile.addEventListener('click',()=>{
      ...
      if(f.id==='tapparelle'){
        const next=!isOn; callAdmin(next?'alza_tutto':'abbassa_tutto',{},()=>{},()=>{}); setShuttersUp(next); setTimeout(loadAll, 800);
      }
    });

    tile.querySelector('.action-btn').addEventListener('click',(ev)=>{
      ev.stopPropagation();
      if(f.id==='tapparelle'){
      ...
      if(f.kind==='action' && f.event){ callAdmin(f.event,{},()=>{},()=>{}); }
    });
...
function loadAll(){ fetchModel((m)=>{ LAST_MODEL=m; try{ renderState(m); renderPeople(m); renderWeather(m); renderEnergy(m); renderFavorites(m); renderVimar(m); ensureWeatherFallback(m); const err = (m.alerts && Number(m.alerts.logErrors)) || 0; const badge = $('#badgeLog'); if (badge) badge.hidden = (err <= 0); const kb = $('#btnKebab'); if (kb) kb.hidden = (err <= 0); const ts=$('#ts'); if(ts) ts.textContent='Aggiornamento: '+fmtTime(m.meta&&m.meta.nowIso); }catch(e){console.error(e);} }, (e)=>{ console.error('Model error',e); }); }
function loadLogs(){ fetchLogs(d=>renderLogs((d&&d.logs)||[]), e=>console.error('Logs error',e)); }

document.addEventListener('DOMContentLoaded',()=>{ $$('.tab').forEach(t=>t.addEventListener('click',()=> activateTab(t.getAttribute('data-target')))); $$('.menu-btn').forEach(b=>b.addEventListener('click',()=> activateTab(b.getAttribute('data-target')))); const btn=$('#btnRefreshLog'); if(btn) btn.addEventListener('click', loadLogs); loadAll(); loadLogs(); if(CONFIG && CONFIG.AUTO_REFRESH_MS>0) setInterval(loadAll, CONFIG.AUTO_REFRESH_MS);
  const go=$('#btnHomeGo'); if(go) go.addEventListener('click',()=> activateTab('home'));
  const kebab=$('#btnKebab'); if(kebab) kebab.addEventListener('click',()=> activateTab('log'));
  const add=$('#btnAdd'); if(add) add.addEventListener('click',()=> activateTab('devices'));
