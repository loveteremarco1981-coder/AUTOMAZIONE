...
// 4 comandi iOS: ssid_on, ssid_off, mark_out, mark_in
// Più: force_in, force_out, set_vacanza, set_override, alza/abbassa, piante, diag
...
function doGet(e){
  ...
  try{
    ...
    if(ev==='ssid_off'){
      if(!who) return out({ok:false,err:'missing_name'});
      ssidOff_(who);
      // NON chiama evaluateStateNow — lo stato non cambia con ssid_off
    ...
    if(ev==='mark_out'){
      if(!who) return out({ok:false,err:'missing_name'});
      markOut_geofence_(who); // chiama evaluateStateNow internamente
      return out({ok:true, name:who, now:new Date().toISOString()});
    }
    ...
    if(ev==='mark_in'){
      if(!who) return out({ok:false,err:'missing_name'});
      markIn_geofence_(who); // chiama evaluateStateNow internamente
      return out({ok:true, name:who, now:new Date().toISOString()});
    }
