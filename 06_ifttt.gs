...
var _IFTTT_ALIAS_={
  ...
  'piante':'piante','spegni_clima':'spegni_clima',
  'log_life':'log_life','log_arrivo':'log_arrivo','log_uscita':'log_uscita'
};

function _iftttSafe_(eventName, payload){
  var key=IFTTT_KEY();
  if(!key){ logEvent('IFTTT_SKIP','key mancante',''); return; }
  try{
    ...
    logEvent('IFTTT_OK',applet,'HTTP '+code+' '+body.substring(0,60));
